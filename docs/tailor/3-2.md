...
!!! note "AI MATURITY CURVE"
    **Rules are committed to the repository.** This is the heart of the ***Tailor*** stage in your progression towards the right-side of the **AI Maturity Curve**.
...
1. Within the Bob IDE, use the **File Explorer** tab to locate **`.bob`** — positioned directly under the `GALAXIUM-TRAVELS` folder header. Right-click **`.bob`** to expose additional options.
...
3. A text box will appear waiting for input. Name the folder **`rules`** and press ++return++ to continue.
...
6. A text box will appear waiting for input. Name the file **`basic_rules.md`** and press ++return++ to continue.
...
8. First, verify that Bob's Agentic Sidebar is still set to **Code mode**. When ready, send the following prompt to the Agentic Sidebar:
...
9. If prompted by Bob along the way, click the blue action button shown— **Save**, **Approve**, **Approve All**, or **Run** —to keep things moving.
...
10. As before, Bob will generate a *To-Do List* as it sets to work.
...
15. As before, Bob will generate a *To-Do List* as it sets to work.

    !!! warning ""
        Take note that the To-Do List now includes the creation of an internal-monologue summary — a telltale sign that your new ruleset is being adhered to.
...
17. Continue through the Agentic Sidebar workflow as you would during a typical interaction.
    ...
    From time to time, a prompt may appear requesting a text response (rather than one of the blue action buttons.) For example, Bob may ask for additional details about how the `README` file should be updated. If you receive a prompt such as this, the specific response is not important for this lab, so feel free to enter any option or details you prefer.
...
18. Eventually, a *diff view* will appear containing the proposed changes to the `README` file.
    ...
    Red text indicates content that is now scheduled to be removed; the highlight color (variable depending on your Bob IDE) indicates new content that is scheduled to be added.
...
19. An internal-monologue (`Interaction Summary`) document is also created exactly as you specified in the `basic_rules.md` file.
...
20. On completion of the workflow, Bob will return a **`Task Completed`** message to the console.
    ...
    - As requested by the "update the `README`" prompt, the `README` file has been modified as an internal-monologue document (adhering to the `yyyy-mm-dd_NAME.md` naming format that was specified in the ruleset)
